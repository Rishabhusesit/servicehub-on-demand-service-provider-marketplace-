...
class ManageUsersController extends Controller {
    ...
    public function detail($id) {
        ...
        $completeOrderAmount = Order::where('user_id', $user->id)->where('status', Status::ORDER_COMPLETED)->sum('total');
        $totalOrder          = Order::where('user_id', $user->id)->count();
        $pendingOrder        = Order::where('user_id', $user->id)->where('status', Status::ORDER_PENDING)->count();
        $completedOrder      = Order::where('user_id', $user->id)->where('status', Status::ORDER_COMPLETED)->count();

        return view('admin.users.detail', compact('pageTitle', 'user', 'pendingOrder', 'completedOrder', 'completeOrderAmount', 'countries', 'totalOrder'));
    }
