...
class FrontendController extends Controller
{
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        if ($type == 'data') {
            $inputContentValue['image'] = isset($content->data_values->image) ? $content->data_values->image : null;
            ...
            if ($imgJson) {
                foreach ($imgJson as $imgKey => $imgValue) {
                    $imgData = isset($request) && is_array($request->image_input) && isset($request->image_input[$imgKey])
                    ? $request->image_input[$imgKey]
                    : null;
                    if (is_file($imgData)) {
                        try {
                            $inputContentValue[$imgKey] = $this->storeImage($imgJson, $type, $key, $imgData, $imgKey, (isset($content->data_values->$imgKey) ? $content->data_values->$imgKey : null ));
                        } catch (\Exception $exp) {
                            $notify[] = ['error', 'Couldn\'t upload the image'];
                            return back()->withNotify($notify);
                        }
    ...
    public function frontendSeoUpdate(Request $request, $key, $id)
    {
        ...
        $data  = Frontend::findOrFail($id);
        $image = isset($data->seo_content->image) ? $data->seo_content->image : null;
    ...
    protected function storeImage($imgJson, $type, $key, $image, $imgKey, $oldImage = null)
    {
        ...
        if ($type == 'element' || $type == 'content') {
            $size  = isset($imgJson->$imgKey->size) ? $imgJson->$imgKey->size : '';
            $thumb = isset($imgJson->$imgKey->thumb) ? $imgJson->$imgKey->thumb : '';
        } else {
            $path  = getFilePath($key);
            $size  = getFileSize($key);
            $thumb = isset(fileManager()->$key()->thumb) ? fileManager()->$key()->thumb : '';
        }
    ...
    public function remove($id)
    {
        ...
        $key      = explode('.', (isset($frontend->data_keys) ? $frontend->data_keys : ''))[0];
        $type     = explode('.', (isset($frontend->data_keys) ? $frontend->data_keys : ''))[1];
        if ($type == 'element' || $type == 'content') {
            $path    = 'assets/images/frontend/' . $key;
            $imgJson = (isset(getPageSections()->$key->$type->images) && getPageSections()->$key->$type->images) ? getPageSections()->$key->$type->images : null;
            if ($imgJson) {
                foreach ($imgJson as $imgKey => $imgValue) {
                    fileManager()->removeFile($path . '/' . isset($frontend->data_values->$imgKey) ? $frontend->data_values->$imgKey : '');
                    fileManager()->removeFile($path . '/thumb_' . isset($frontend->data_values->$imgKey) ? $frontend->data_values->$imgKey : '');
                }
            }
            if (isset(getPageSections()->$key->element->seo) && getPageSections()->$key->element->seo) {
                fileManager()->removeFile($path . '/seo/' . isset($frontend->seo_content->image) ? $frontend->seo_content->image : '');
            }
        }
        $frontend->delete();
