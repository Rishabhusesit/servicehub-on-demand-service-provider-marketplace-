...
class AdminController extends Controller {
    public function dashboard() {
        ...
        $widget['total_providers']             = Provider::count();
        $widget['verified_providers']          = Provider::active()->count();
        $widget['email_unverified_providers']  = Provider::emailUnverified()->count();
        $widget['mobile_unverified_providers'] = Provider::mobileUnverified()->count();

        $widget['active_coupon'] = Coupon::where('status', Status::ACTIVE)->whereDate('start_date', '<=', now())->whereDate('end_date', '>=', now())->count();
        $widget['category']      = Category::active()->count();
        $widget['city']          = City::active()->count();
        $widget['area']          = Area::active()->count();

        $orders['all']        = Order::count();
        $orders['pending']    = Order::pending()->count();
        $orders['processing'] = Order::processing()->count();
        $orders['requested']  = Order::requested()->count();
        $orders['completed']  = Order::completed()->count();
        $orders['cancelled']  = Order::cancelled()->count();
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'withdrawals', 'orders'));
    }
    ...
    public function requestReport() {
        ...
        $statusCondition = !isset($response->status) || (isset($response->status) && !$response->status);
        $messageCondition = !isset($response->message) || (isset($response->message) && !$response->message);
        if (!$response || $statusCondition || $messageCondition) {
