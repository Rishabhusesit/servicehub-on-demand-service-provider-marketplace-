...
class ResetPasswordController extends Controller
{
    public function showResetForm(Request $request, $token = null)
    {
        ...
        if (ProviderPasswordReset::where('token', $token)->where('email', $email)->count() != 1) {
            $notify[] = ['error', 'Invalid token'];
            return to_route('provider.password.request')->withNotify($notify);
        }
        return view('Template::provider.auth.passwords.reset')->with(
    ...
    public function reset(Request $request)
    {
        ...
        $reset = ProviderPasswordReset::where('token', $request->token)->orderBy('created_at', 'desc')->first();
        if (!$reset) {
            $notify[] = ['error', 'Invalid verification code'];
            return to_route('provider.login')->withNotify($notify);
        }

        $user           = Provider::where('email', $reset->email)->first();
