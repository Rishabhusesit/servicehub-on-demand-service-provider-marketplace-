...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $btcrate = (isset($res->USD->last) && $res->USD->last) ? $res->USD->last : 0;
        ...
        if ($deposit->btc_amount == 0 || $deposit->btc_wallet == "") {
            ...
            if (!isset($response->address) || (isset($response->address) && $response->address == '')) {
                $send['error'] = true;
                $send['message'] = 'BLOCKCHAIN API HAVING ISSUE. PLEASE TRY LATER. ' . (isset($response->message) ?$response->message : '');
            } else {
