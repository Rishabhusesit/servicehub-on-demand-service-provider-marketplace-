...
function loadCustomCaptcha($width = '100%', $height = 46, $bgColor = '#f4f4f4') {
...
function getTemplates() {
    ...
    $param['website']      = $host . $uri . ' - ' . env('APP_URL');
...
function menuActive($routeName, $type = null, $param = null) {
    ...
    if (is_array($routeName)) {
        ...
        if ($param) {
            ...
            if (strtolower(isset($routeParam[0]) ? $routeParam[0] : null) == strtolower($param)) {
                return $class;
            } else {
                return;
            }
...
function formatOfferDiscount($offer) {
    ...
    if ($offer->discount_type == 1) {
        return 'Flat ' . gs('cur_sym') . getAmount($amount) . ' Discount';
    } else {
        return getAmount($amount) . '% Discount';
    }
...
function initializePusher() {
    ...
    Config::set('broadcasting.connections.pusher.key', gs('pusher_app_key'));
    Config::set('broadcasting.connections.pusher.secret', gs('pusher_app_secret'));
    Config::set('broadcasting.connections.pusher.app_id', gs('pusher_app_id'));
    Config::set('broadcasting.connections.pusher.options.cluster', gs('pusher_app_cluster'));
}
