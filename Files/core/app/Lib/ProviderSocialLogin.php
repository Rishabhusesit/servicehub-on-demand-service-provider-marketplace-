...
class ProviderSocialLogin
{
    ...
    private function configuration()
    {
        ...
        Config::set('services.' . $provider, [
            ...
            'redirect'      => route('provider.social.login.callback', $provider),
        ]);
    }
    ...
    public function login()
    {
        ...
        $userData = Provider::where('provider_id', $user->id)->first();

        if (!$userData) {
            if (!gs('registration')) {
                throw new Exception('New account registration is currently disabled');
            }
            $emailExists = Provider::where('email', (isset($user->email) ? $user->email : ''))->exists();
            if ($emailExists) {
    ...
    private function createUser($user, $provider)
    {
        ...
        $adminNotification->provider_id = $newUser->id;
        $adminNotification->title       = 'New member registered';
        $adminNotification->click_url   = urlPath('admin.providers.detail', $newUser->id);
        $adminNotification->save();

        $user = Provider::find($newUser->id);
