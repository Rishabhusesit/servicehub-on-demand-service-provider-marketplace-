        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    ...
                    @if($provider->kv == Status::KYC_UNVERIFIED)
                    <div class="my-3">
                        <h6>@lang('Rejection Reason')</h6>
                        <p>{{ $provider->kyc_rejection_reason }}</p>
                    </div>
                    @endif

                    @if($provider->kv == Status::KYC_PENDING)
                    <div class="d-flex flex-wrap justify-content-end mt-3">
                        ...
                        <button class="btn btn-outline--success confirmationBtn" data-question="@lang('Are you sure to approve this documents?')" data-action="{{ route('admin.providers.kyc.approve', $provider->id) }}"><i class="las la-check"></i>@lang('Approve')</button>
                    </div>
                    @endif
                </div>
            </div>
    ...
    <div id="kycRejectionModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.providers.kyc.reject', $provider->id) }}" method="POST">
                    @csrf
